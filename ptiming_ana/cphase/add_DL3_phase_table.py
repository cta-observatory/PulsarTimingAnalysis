...
def main():
    ...
    parser.add_argument(
        ...
        dest="directory",
        default=None,
        help="Directory where to find the DL3 files",
    )
    parser.add_argument(
        "--in-file",
        "-f",
        ...
        dest="in_file",
        default=None,
        help="DL3 standard file if want to analyze only one file",
    )
    parser.add_argument(
        "--ephem",
        "-ephem",
        ...
        dest="ephem",
        default=None,
        help="Path to the ephemeris file (.par or .gro)",
    )
    parser.add_argument(
        "--output",
        "-out",
        ...
        dest="dir_output",
        default=None,
        help="Path where to store the DL3 file with the pulsar info",
    )
    parser.add_argument(
        "--pickle",
        "-pickle",
    ...
    parser.add_argument(
        ...
        dest="run",
        default=False,
        help="Run number to process (only if --dir is given)",
    )
    parser.add_argument(
        ...
        dest="ninterp",
        default=1000,
        help="Number of events between two interpolation points.",
    )
    parser.add_argument(
        "--observatory",
        "-obs",
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="interpolation",
        help="Set to True if want to use the interpolation method (faster but loses some precision)",
    )
    parser.add_argument(
        "--create-tim",
        "-tim",
        action="store_true",
        dest="create_tim",
        help="Set True to create and save a .tim file",
    )
    ...
    if output_dir is None:
        warnings.warn(
            "WARNING: No output directory is given so the output will not be saved"
        )
